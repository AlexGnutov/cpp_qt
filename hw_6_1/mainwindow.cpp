...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        ...
        ui->te_debug->append("Запускаю код на основе QtConcurrent");
        //Тут должен быть код ДЗ

        auto race1 = [this]() {
            concurRace1->DoWork(&number, true, ui->sb_initNum->value());
        };
        auto race1Future = QtConcurrent::run(race1);

        auto race2 = [this]() {
            concurRace2->DoWork(&number, true, ui->sb_initNum->value());
        };
        auto race2Future = QtConcurrent::run(race2);
